...
namespace UI
{
    public class UIService : MonoGenericSingleton<UIService>
    {
        ...
        public void UpdateFuelUI(float fuelToDisplay)
        {
            ...
            fuelText.SetText("Baterai Drone mu : " + fuelToDisplay.ToString("0") + "%");

            if(fuelToDisplay <= 0)
            {
                fuelToDisplay = 0;
                fuelText.SetText("Anda kehabisan baterai !");
            }
        ...
        {
            toatalPackageDeliveredText.SetText("Paket Telah Dikirim : " + packageDelivered.ToString());
        }

        public void UpdateDeliveryTimerText(string text)
        {
            // Auto ganti "Remaining Time" ke "Waktu Tersisa"
            text = text.Replace("Remaining Time", "Waktu Tersisa");
        ...
        private void Pause()
        {
            ...
            gameMenuText.SetText("Permainan Dijeda");
            SetGameMenuUIActive(true);
        ...
        {
            SoundService.Instance.PlaySoundEffects(SoundType.Button);
            gameMenuText.SetText("Permainan Berakhir");
            SetGameMenuUIActive(false);
